...
constructor():
  ...
  hostNode = render()
  if (hostNode) hostNode.appendChild(hostNode)
...
### `packages/core/src/lyn-hostNode.ts`
...
export class LynElement extends HTMLElement {
  ...
  constructor() {
    ...
    // 초기 렌더 결과를 루트에 부착 (1회)
    const hostNode = this.render();
    if (hostNode) this.hostNode.appendChild(hostNode);
  }
...
```ts
export { LynElement } from './lyn-hostNode';
```
