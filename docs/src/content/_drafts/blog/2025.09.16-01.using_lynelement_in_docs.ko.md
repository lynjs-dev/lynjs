...
`customElements.define`을 통해 브라우저에 등록하면, 이제 HTML 어디서든 `<my-hostNode>`라는 태그로 이 컴포넌트를 사용 할
수 있다.
...
customElements.define('my-hostNode', MyElement);
```
...
- `customElements.define('my-hostNode', MyElement)`를 통해 `<my-hostNode>`라는 태그 이름을 브라우저에 등록한다.

**/docs/src/content/docs/en/api/components/define.html**

```html
<my-hostNode></my-hostNode>
```

이 예제 파일을 mdx파일 에서 import 하고 `<my-hostNode />`를 작성해 실행 결과를 확인할 수 있기를 기대한다.
...
## 문서에서 Custom hostNode 사용 (`define.md`)
...
### define.mdx에서 custom hostNode 사용

preview에는 <my-hostNode></my-hostNode> 같은 사용 예 HTML이 들어간다. 이 HTML 요소가 정상적으로 동작하려면 브라우저
