...
필요하다. hostNode.js 생태계에는 Jest, Mocha, Jasmine 같은 다양한 테스트 도구가 있다. 그중 **Vitest**를 하려고 한다.
...
`packages/core/vite.config.js` 파일을 생성한다.
...
{
  "scripts": {
    "test:core": "npm --prefix packages/core run core",
    "test:core:watch": "npm --prefix packages/core run core:watch",
    "test:core:browser": "npm --prefix packages/core run core:browser"
  }
}
```
...
**packages/core/test/lyn-hostNode.define.test.tsx**
...
describe('LynElement - define', () => {
  const tag = 'x-core-define-lyn-hostNode';
  ...
  it('should create hostNode via document.createElement and upgrade to LynElement', () => {
...
- `customElements.define(tag, XTestEl)` → 브라우저에 `x-test-define-lyn-hostNode` 태그를 등록한다.
