...
이 과정을 거쳐야 `<my-hostNode></my-hostNode>`라는 태그가 HTML 안에서 인식된다. 정의하지 않으면 브라우저는 단순한
“이름만 긴 태그”로 취급한다.
...
1. 생성 (constructor)
   - 자바스크립트에서 `document.createElement('my-hostNode')` 또는 DOM 파서가 `<my-hostNode>`를 만났을 때 호출된다.
